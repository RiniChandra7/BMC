...
public class SchemeDetailQueryBuilder {
    ...
    private static final String FROM_TABLES = """ 
            from eg_bmc_schemeevent schev \
            left join eg_bmc_schemes sch on schev.schemeid = sch.id \
            left join eg_bmc_event ev on schev.eventid = ev.id \
            """;
        ...
        if (!ObjectUtils.isEmpty(criteria.getStatus())) {
            ...
            switch(criteria.getStatus())
            {
                ...
                case FUTURE:
                        ...
                        break;
                case PRESENT:
                        query.append(" to_timestamp(ev.startdt)::date <= current_date and to_timestamp(coalesce(ev.enddt,4102444799))::date >= current_date ");
                        addClauseIfRequired(query, preparedStmtList);
                        break;
                default:
